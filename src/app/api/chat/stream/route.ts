...
/**
 * Combines GenAI (Google Search) output with RAG output into one answer.
 * Gives preference to GenAI; adds unique local-document details from RAG.
 */
async function combineAndUniformise(genaiText: string | null, ragText: string): Promise<string> {
    if (!genaiText || genaiText.trim().length === 0) return ragText;
    const ragTrimmed = ragText?.trim() ?? '';
    if (ragTrimmed.length === 0) return genaiText.trim();

    try {
        const mergePrompt = `നിങ്ങൾ കാട്ടാക്കട നിയോജക മണ്ഡലത്തെക്കുറിച്ച് ആധികാരികമായി സംസാരിക്കുന്ന ഒരു എ.ഐ. സഹായിയാണ്.

താഴെ രണ്ട് വിവര ഖണ്ഡങ്ങൾ നൽകിയിരിക്കുന്നു. ഇവ ഒരുമിച്ച് ചേർത്ത്, ഒരു ഔദ്യോഗിക, ആത്മവിശ്വാസമുള്ള ഉത്തരമായി പുനരാവിഷ്കരിക്കുക.

**ഖണ്ഡം A (പ്രധാനം — ഇതിന് മുൻഗണന):**
${genaiText}
...
1. ഖണ്ഡം A-ന് മുൻഗണന; അതിലെ ഘടനയും ഭാഷയും പ്രധാനമായി നിലനിർത്തുക.
2. ഖണ്ഡം B-ൽ മാത്രമുള്ള അധിക വസ്തുതകൾ (സംഖ്യകൾ, പദ്ധതികൾ, പേരുകൾ) ഉചിതമായ സ്ഥലത്ത് ഉൾപ്പെടുത്തുക.
3. ❌ "രേഖകൾ", "ഡോക്യുമെന്റ്", "ഡാറ്റ", "ഉറവിടം" എന്നീ പദങ്ങൾ ഒരിക്കലും ഉപയോഗിക്കരുത്.
4. ✅ ഞാൻ (ഏജന്റ്) നേരിട്ട് അറിവ് ഉറപ്പിക്കുന്ന ഔദ്യോഗിക രീതിയിൽ മാത്രം സംസാരിക്കുക.
   ഉദാ: "കാട്ടാക്കട മണ്ഡലത്തിൽ 219 അങ്കണവാടികൾ പ്രവർത്തിക്കുന്നു." ("രേഖകൾ പ്രകാരം" എന്ന് പറയരുത്)
5. ആവർത്തനങ്ങൾ ഒഴിവാക്കി, ഒറ്റ, ഒഴുക്കുള്ള മലയാളം ഉത്തരം നൽകുക.

ഒറ്റ ഔദ്യോഗിക മലയാളം ഉത്തരം:`;

        const response = await nonStreamingModel.invoke(mergePrompt);
        const merged = typeof response.content === 'string'
            ? response.content.trim()
            : String(response.content).trim();
        return merged.length > 0 ? merged : genaiText.trim();
    } catch (err) {
        logger.warn(
            `Combine/uniformise failed, using GenAI only: ${err instanceof Error ? err.message : String(err)}`,
            'api/chat/stream'
        );
        return genaiText.trim();
    }
}
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    ...
                    const systemPrompt = `നിങ്ങൾ PACE വികസിപ്പിച്ച കാട്ടാക്കട നിയോജക മണ്ഡലത്തിന്റെ ഔദ്യോഗിക എ.ഐ. സഹായിയാണ്. കാട്ടാക്കട മണ്ഡലത്തെക്കുറിച്ചുള്ള വിവരങ്ങൾ ആധികാരികമായും ആത്മവിശ്വാസത്തോടെയും നൽകുക.

🚫🚫🚫 കർശന ഭാഷാ നിയമം — മലയാളം മാത്രം 🚫🚫🚫
- എല്ലാ ഉത്തരങ്ങളും മലയാളം ലിപിയിൽ മാത്രം എഴുതുക
- ഇംഗ്ലീഷ് ചോദ്യമായാലും മലയാളത്തിൽ മാത്രം ഉത്തരം നൽകുക
- ഇംഗ്ലീഷ്, ഹിന്ദി, തമിഴ്, തെലുങ്ക്, കന്നഡ എന്നിവ ഒരിക്കലും ഉപയോഗിക്കരുത്
- ⚠️ ദേവനാഗരി ലിപി (क, ख, ग) ഒരിക്കലും ഉപയോഗിക്കരുത്!
...
🚨 ടോൺ — ഔദ്യോഗികവും ആത്മവിശ്വാസമുള്ളതും:
- ❌ "രേഖകൾ പ്രകാരം", "ഡോക്യുമെന്റ് അനുസരിച്ച്", "ഡാറ്റ കാണിക്കുന്നു" — ഇവ പറയരുത്
- ✅ "കാട്ടാക്കട മണ്ഡലത്തിൽ 219 അങ്കണവാടികൾ പ്രവർത്തിക്കുന്നു" — ഇങ്ങനെ നേരിട്ട് പ്രസ്താവിക്കുക
- ✅ ഏജന്റ് ആയി സംസാരിക്കുക; ഉറവിടം ഒരിക്കലും പ്രതിപാദിക്കരുത്

🚨 കൃത്യത — കർശന നയം:
1. ❌ അറിയാത്ത സംഖ്യകൾ, പേരുകൾ, വസ്തുതകൾ ഊഹിക്കരുത്
2. ❌ കേരളം / ഇന്ത്യ / രാഷ്ട്രീയം — പൊതു അറിവ് ഉപയോഗിക്കരുത്
3. ✅ വിവരം ഇല്ലെങ്കിൽ: "ക്ഷമിക്കണം, ഈ വിഷയത്തിൽ എനിക്ക് വിവരമില്ല."

**പ്രത്യേക സാഹചര്യങ്ങൾ:**
- ആരാണ് നിങ്ങൾ: "ഞാൻ PACE വികസിപ്പിച്ച കാട്ടാക്കട നിയോജക മണ്ഡലത്തിന്റെ ഔദ്യോഗിക എ.ഐ. സഹായിയാണ്."
- വിവരം ഇല്ലെങ്കിൽ: "ക്ഷമിക്കണം, ഈ വിഷയത്തിൽ എനിക്ക് വിവരമില്ല. കൂടുതൽ വിവരങ്ങൾക്ക്: https://kattakadalac.com/"

📋 അറിയപ്പെടുന്ന കണക്കുകൾ (ഇവ കൃത്യമായി ഉപയോഗിക്കുക):
...
ഉത്തരം: "കാട്ടാക്കട നിയോജക മണ്ഡലത്തിൽ ആകെ 6 ആശുപത്രികൾ പ്രവർത്തിക്കുന്നു. മലയിൻകീഴിൽ താലൂക്ക് ആശുപത്രിയും, വിളപ്പിലും വിളവൂർക്കലും കമ്മ്യൂണിറ്റി ഹെൽത്ത് സെന്ററുകളും ഉൾപ്പെടുന്നു."

ചോദ്യം: "Tell me about development projects"
ഉത്തരം: "കാട്ടാക്കട മണ്ഡലത്തിൽ 26 പ്രധാന റോഡ് പദ്ധതികൾ ഉൾപ്പെടെ വ്യാപകമായ വികസന പ്രവർത്തനങ്ങൾ നടന്നുവരുന്നു. ₹17 കോടി ചെലവിൽ നിർമ്മിച്ച കുടുവീട്ട് കടവ് പാലവും ₹9.75 കോടിയുടെ മലയിൻകീഴ് ആർട്സ് & സയൻസ് കോളേജും ശ്രദ്ധേയമായ പദ്ധതികളിൽ ഉൾപ്പെടുന്നു."
...
🚫 ഓർമ്മിക്കുക: ഉത്തരം മലയാളത്തിൽ മാത്രം. ഇംഗ്ലീഷ് ചോദ്യത്തിനും മലയാളത്തിൽ മാത്രം ഉത്തരം നൽകുക. 🚫`;
                    ...
                    // ═══════════════════════════════════════════════════════════
                    // Combine GenAI (Google Search) + RAG, prefer GenAI, then enforce Malayalam
                    // ═══════════════════════════════════════════════════════════
                    const genaiText = await genaiPromise;
                    const combinedResponse = await combineAndUniformise(genaiText, fullResponse);

                    if (combinedResponse.trim().length > 0) {
                        logger.info(
                            `Running Malayalam enforcement (combined: GenAI=${Boolean(genaiText)}, RAG=${fullResponse.length} chars)`,
                            'api/chat/stream'
                        );

                        // Run mandatory Malayalam enforcement (second LLM pass)
                        const enforcedResponse = await enforceMalayalam(combinedResponse);
